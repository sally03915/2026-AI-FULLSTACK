...
1. 초기화순서
    ...
    4-1)   [    ]   : String ,객체 - null /  int - 0 으로 초기화
    4-2)   [    ]   :  int a=10;    중요콘텐츠 명시적으로 알림!    
    4-3)   [    ]   :  { a=10; b=20; }   여러개초기화시
    4-4)   [    ]    : 최종은 생성자에서 사용함.  인스턴스변수 초기화


2. runtime data area
 [    ]  : 정보저장 , static, final
 [    ]  : 동적저장 - new ,  gc( garbage collecetor)가 처리소멸
 [    ]  : 임시값저장

3. static
- jvm 소스로딩시 메모리 할당받음
- new연산자보다 먼저 실행되어 메모리(method 영역:runtime)에    (        )    회 생성
...
오버로딩은 같은 클래스 내에서 (㉙ 메서드이름   )이 같고 (㉚ 파라미터   )이 다른 메서드를 정의하는 것.
